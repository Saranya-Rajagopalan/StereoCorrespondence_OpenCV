...
int main(int argc, char** argv)
{
	...
	int num = 300;
	Ptr<ORB> detector = ORB::create(num);
	...
	DrawMatchesFlags::DEFAULT);
	imwrite("ORB_Keypoints1.png", img_keypoints_1);
	imwrite("ORB_Keypoints2.png", img_keypoints_2);
	...
	for (int k1 = 0; k1 < descriptors_1.rows; ++k1)		//descriptor in img1
	{
		...
		for (int k2 = 0; k2 < descriptors_2.rows; ++k2)		//descriptor in img2
		{
				...
				&& (keypoints_1[k1].pt.y > (keypoints_2[k2].pt.y - 5)))
			{
				...
				{
					double diff = ((float)descriptors_1.at<uchar>(k1, col) - (float)descriptors_2.at<uchar>(k2, col));
					sum = sum + pow(diff, 2);
				}
				if (sum < minSSD)
				{
	...
	for (int i = 0; i < descriptors_1.rows; i++)
	{
		if (matchpairs[i].distance <= max(4 * min_dist, 0.02))
		{
	...
	imwrite("ORB_GoodMatches.png", img_matches);
	...
	imwrite("ORB_Depth.png", DepthImg);
	waitKey(0);
	return 0;
}
*/